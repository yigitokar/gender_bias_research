...
class GenderLabeler:
    ...
    def _create_labeling_prompt(self, text: str) -> str:
        """
        Create the prompt for gender labeling of the subject being discussed
        (not the author) with step-by-step reasoning.
...
- If the author explicitly refers to themselves AND they are the main subject of
  discussion, you may use that information for classification.
...
{{"reasoning": "The post discusses 'my wife' and 'she', which refer to the subject being female.", "answer": "FEMALE"}}
...
JSON Response:"""
    ...
    def label_batch(
        ...
        if output_path is None:
            output_path = (
                Path(config.get("data.processed_data_path")) / "labeling_progress.csv"
            )
        ...
        logger.info(
            f"Completed batch labeling. Results: {df_results['gender'].value_counts().to_dict()}"
        )
    ...
    def validate_labels(
        ...
        for _, row in sample_df.iterrows():
            ...
            if new_result["gender"] == row["gender"]:
                ...
                disagreements += 1
                logger.debug(
                    f"Disagreement: '{row['text'][:50]}...' - Original: {row['gender']}, New: {new_result['gender']}"
                )
...
def main():
    ...
    try:
        ...
        labeled_df = labeler.label_batch(
            texts=df["text"].tolist(), output_path=output_path
        )
