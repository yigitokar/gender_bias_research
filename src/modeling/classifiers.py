...
class EmbeddingClassifier:
    ...
    def __init__(self, random_state: int = None):
        ...
        self.random_state = random_state or classification_config.get(
            "random_state", 42
        )
    ...
    def _initialize_models(self) -> dict[str, Any]:
        ...
        # Lasso Logistic (per paper). lambda=0.001 -> C ~ 1000 (inverse reg strength)
        ...
        # MLP (Torch) matching paper architecture for binary FEMALE/MALE
        models["MLP_Torch"] = "torch_mlp"

        # LightGBM (optional; skip if not installed)
        try:
    ...
    def prepare_data(
        ...
        logger.info(
            f"Prepared data: Train: {X_train_scaled.shape}, Test: {X_test_scaled.shape}"
        )
    ...
    def train_models(
        ...
        for name, model in self.models.items():
            ...
            try:
                if name == "MLP_Torch":
                    # Create a small validation split from training for early selection
                    from sklearn.model_selection import train_test_split
                ...
                if use_cross_validation and name != "MLP_Torch":
                ...
                logger.info(
                    f"{name} training complete. CV accuracy: {cv_mean:.4f} ± {cv_std:.4f}"
                )
    ...
    def compare_embeddings(
        self, embedding_datasets: dict[str, pd.DataFrame], target_column: str = "gender"
    ) -> dict[str, dict[str, float]]:
        """
        ...
        for embedding_name, df in embedding_datasets.items():
            ...
            try:
                ...
                logger.info(
                    f"{embedding_name}: Best model {best_model} - "
    ...
    def hyperparameter_tuning(
        ...
        try:
            ...
            logger.info(
                f"Hyperparameter tuning complete. Best score: {grid_search.best_score_:.4f}"
            )
    ...
    def save_models(self, output_dir: str) -> None:
        ...
        try:
            ...
            for name, model in self.trained_models.items():
                # Torch models: save state_dict instead of pickling the whole module
                if isinstance(model, nn.Module) or name == "MLP_Torch":
                    model_path = (
                        output_path / f"{name.replace(' ', '_').lower()}_state.pt"
                    )
                    torch.save(model.state_dict(), model_path)
                else:
                    model_path = (
                        output_path / f"{name.replace(' ', '_').lower()}_model.pkl"
                    )
