...
class OpenAIEmbedder:
    ...
    def __init__(self, api_key: str = None, model_name: str = None):
        ...
        self.model_name = model_name or api_config.get(
        ...
        logger.info(
            f"Initialized OpenAIEmbedder with model: {self.model_name} ({self.embedding_dim}d)"
        )
    ...
    def embed_batch(
        ...
        if progress_file is None:
            progress_file = (
                Path(config.get("data.embeddings_path"))
                / "openai_embedding_progress.csv"
            )
        ...
        if resume_from_checkpoint and save_progress and Path(progress_file).exists():
            try:
                ...
                embeddings = [
                    np.array(row) for row in progress_df.filter(regex="^embed_").values
                ]
                logger.info(f"Resuming from checkpoint at index {start_idx}")
            except Exception as e:
                logger.warning(
                    f"Could not load progress file: {e}. Starting from beginning."
                )
                start_idx = 0
                embeddings = []

        logger.info(
            f"Starting OpenAI embedding generation for {total_texts} texts (from index {start_idx})"
        )
        ...
        for i in range(start_idx, total_texts):
            try:
                ...
                if (i + 1) % 100 == 0:
                    ...
                    logger.info(
                        f"Processed {i + 1}/{total_texts} texts ({progress:.1f}%)"
                    )

                # Save progress periodically
                if save_progress and (i + 1) % batch_size == 0:
                    self._save_embedding_progress(
                        texts[: i + 1], embeddings, progress_file
                    )
    ...
    def _save_embedding_progress(
        self, texts: list[str], embeddings: list[np.ndarray], progress_file: str
    ) -> None:
        """
        ...
        try:
            ...
            if embeddings:
                ...
                progress_df = pd.concat([pd.DataFrame(data), embedding_df], axis=1)
            else:
    ...
    def compare_with_alternatives(
        self, texts: list[str], sample_size: int = 50
    ) -> dict[str, Any]:
        """
        ...
        logger.info(
            f"OpenAI embedding analysis: avg norm: {analysis['avg_norm']:.3f}, "
