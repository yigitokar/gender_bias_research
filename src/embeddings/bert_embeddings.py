...
class BertEmbedder:
    ...
    def __init__(self, model_name: str = None, device: str = None):
        ...
        self.model_name = model_name or bert_config.get(
    ...
    def embed_batch(
        ...
        logger.info(
            f"Starting {strategy.upper()} embedding generation for {total_texts} texts"
        )

        # Choose embedding function
        embed_func = (
            self.get_cls_embedding if strategy == "cls" else self.get_mean_embedding
        )
        ...
        for i in range(0, total_texts, batch_size):
            ...
            if show_progress and (i + batch_size) % (batch_size * 10) == 0:
                ...
                logger.info(
                    f"Processed {min(i + batch_size, total_texts)}/{total_texts} texts ({progress:.1f}%)"
                )

        embeddings_array = np.vstack(embeddings)
        logger.info(
            f"Generated {strategy.upper()} embeddings: {embeddings_array.shape}"
        )
    ...
    def embed_dataframe(
        ...
        embeddings = self.embed_batch(
            texts=df[text_column].fillna("").tolist(), strategy=strategy
        )
        ...
        embedding_df = pd.DataFrame(
            embeddings,
            columns=[
                f"{output_prefix}_{strategy}_embed_{i}"
    ...
    def compare_strategies(self, texts: list[str], sample_size: int = 100) -> dict:
        """
        ...
        cls_embeddings = self.embed_batch(
            sample_texts, strategy="cls", show_progress=False
        )
        mean_embeddings = self.embed_batch(
            sample_texts, strategy="mean", show_progress=False
        )
        ...
        for i in range(len(sample_texts)):
            ...
            similarity = np.dot(cls_vec, mean_vec) / (
                np.linalg.norm(cls_vec) * np.linalg.norm(mean_vec)
            )
        ...
        logger.info(
            f"Strategy comparison: CLS avg norm: {comparison_stats['cls_avg_norm']:.3f}, "
...
def main():
    ...
    try:
        ...
        # Compare strategies
        comparison = embedder.compare_strategies(
            df["scrubbed_text"].fillna("").tolist()[:100]
        )
